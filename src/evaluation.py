from sklearn.metrics import balanced_accuracy_score, confusion_matrix, ConfusionMatrixDisplay, roc_auc_score, classification_report, precision_score, recall_score
import logging
import pandas as pd
...
def evaluation(X_train:pd.DataFrame, X_test:pd.DataFrame, y_train:pd.Series, y_test:pd.Series, pipeline_final:Pipeline) -> dict[str, float]:
    ...
    metrics = {"balanced_accuracy_train":balanced_accuracy_train, "roc_train": roc_train, "precision_train": precision_train, "recall_train": recall_train, "balanced_accuracy_test": balanced_accuracy_test, "roc_test": roc_test, "precision_test":precision_test, "recall_test": recall_test}
